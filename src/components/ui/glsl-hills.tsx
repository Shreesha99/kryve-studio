...
void main(){
  ...
  float s = sin(radians(p.x / 720.0 * 90.0));
...
varying vec3 vPosition;
void main(){
    float o = smoothstep(140.0, 0.0, length(vPosition)) * 0.55;

  gl_FragColor=vec4(vec3(0.6),o);
}`,
            transparent: true,
          })
        );
    ...
    const loop = () => {
      ...
      plane.render(Math.min(clock.getDelta(), 0.033));
